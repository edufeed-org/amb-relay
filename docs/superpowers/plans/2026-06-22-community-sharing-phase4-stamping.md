...
>
> **Query stamped content via NIP-50 `community:X`, not `#h:X`.** Stamping denormalizes the community into the relay-side `community` index field and cannot add an `h` tag to the signed content event (that would break the author's signature). The relay maps `#h:X` to `community:=X` and returns stamped content, but clients that re-validate tag filters against raw event tags (`fiatjaf.com/nostr`, go-nostr/`nak`) discard it for lack of an `h:X` tag. The `search:"community:X"` filter carries no tag to re-validate, so it surfaces stamped content on every client. `#h:X` reliably returns only Model-A (own-`h`-tag) content. *(Confirmed by live E2E 2026-06-23: stamping verified via `community:X`; the two integration bugs found — `sharesFor` querying `nostr_a` instead of `refA`, and `per_page=10000` returning a 422 inside the 200-OK multi_search envelope — are fixed.)*
...
- [x] **Step 3: Delete the throwaway test.** `rm` it; confirm `git status` shows only intended changes.

- [x] **Step 4: Final docs check** — CLAUDE.md "Community shares" + "Canonical query shapes" updated to make NIP-50 `search:"community:X"` the canonical query for stamped content (the "shared with a community" shape now includes member-shared content, not only directly-`h`-tagged content) and to document why strict `#h:X` does not reliably return Model-B (stamped) content — the signed event carries no `h` tag, so re-validating clients drop it.
