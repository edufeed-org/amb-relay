...
> **Query with NIP-50 `community:X`, not a `#h:X` tag filter.** Stamping denormalizes the community into the relay-side `community` index field; it does **not** (and cannot) add an `h` tag to the content event, since that would invalidate the author's signature. The relay maps a `#h:X` REQ to `community:=X` and *does* return stamped content — but clients that re-validate tag filters against raw event tags (e.g. `fiatjaf.com/nostr`, go-nostr/`nak`) discard it, because the signed event carries no `h:X` tag. The NIP-50 `search:"community:X"` filter carries no tag for clients to re-validate, so it surfaces stamped content on every client. Strict `#h:X` only reliably returns Model-A content (the event's own `h` tag); Model-B (stamped) content is reachable solely via `community:X`.
