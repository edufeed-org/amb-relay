...
- `QUERY_FETCH_TIMEOUT_MS`: Milliseconds a single REQ's search fetch — and, identically, a NIP-45 COUNT — is allowed to run before the relay stops waiting (default `20000`, i.e. 20s, when unset). Hardens against a degraded/lagging Typesense: each backend call already has its own client-side timeout, but a kind-less filter fans out across every registered content type SERIALLY (`registry.fetch`/`registry.count`), so on one shared Typesense instance a genuinely slow (not fast-erroring) backend can otherwise stack up to `N × (per-call timeout)` before the request ships — several minutes at today's registration count, indistinguishable from a client hang. `boundedSeq` wraps the composed `relay.QueryStored` result so REQ always gets EOSE (with whatever partial results arrived in time) within budget; `boundedCount` (both `query_budget.go`) wraps `relay.Count` the same way, replying with a NOTICE + a 0 count on expiry since a scalar count has nothing partial to return. A fast clean error (e.g. Typesense's 503 "Not Ready or Lagging") already terminates well under the budget on its own. **Disable semantics:** `"0"` is an explicit, deliberate opt-out (no cap) and is NOT the same as leaving the variable unset — unset, negative, or non-numeric all fall back to the 20s default.

Optional chunk re-ranking (chunk-rerank + snippet layer lives in nostrlib `khatru/semantic`): when `CHUNK_RERANK_ENABLED=true` and `INDEXER_API_TOKEN` is set, NIP-50 searches are re-ranked by the best matching fulltext passage from amb-indexer's `POST /search_chunks` (`INDEXER_BASE_URL`, default `http://amb-indexer:8080`). Each `/search_chunks` call is bounded by `CHUNK_RERANK_TIMEOUT_MS` (default `5000`); the budget must clear the indexer's query-embed step (1-2s on a CPU embed service) — too tight a budget cancels legitimate warm searches mid-embed, surfacing as indexer 500s and silent fallback. Falls back to plain Typesense search on any indexer error or empty chunk result, so recall never degrades. Negentropy syncs carry no search field and bypass it naturally. Ranking is Reciprocal Rank Fusion of the chunk ranking with the relay's own lexical ranking; a parent missing from one ranking is charged one shared rank (one past the longer list — the chunk list is always 200 deep because vector kNN always returns k, the lexical list only as long as the term has matches). The indexer's per-hit `keyword_matched` flag (`text_match > 0`) is what separates a genuine passage from kNN padding: a parent with no keyword-matched chunk and no lexical hit is not a candidate, while a lexical hit keeps its vector-only passages as its snippet. Without the flag (older indexer) every chunk parent is a candidate, as before. Both rules trace to the OERSI `frieden` incident (2026-09-14), where Cloudflare challenge-page chunks ranked above every keyword hit. Searches that additionally opt in with `kinds:[30142,21142]` receive an
